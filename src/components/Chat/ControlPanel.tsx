...
const ControlPanel = ({ toggleSetting }: { toggleSetting: () => void }) => {
  ...
  return (
    <div className="bottom-0">
      <div className="border-gray-700 border-b border-t-0 border-x-0 border-solid"></div>
      ...
      <div className="border-gray-700 border-b border-t-0 border-x-0 border-solid">
        ...
        <div className="text-right right-0 flex justify-end">
          <IconButton onClick={() => toggleMute()}>
            {isMuted ? (
              <MicOffIcon className="m-auto" />
            ) : (
              <MicIcon className="m-auto" />
            )}
          </IconButton>
          <IconButton onClick={() => toggleDeafen()}>
            {isDeafen ? (
              <HeadsetOffIcon className="m-auto" />
            ) : (
              <HeadsetIcon className="m-auto" />
            )}
          </IconButton>
